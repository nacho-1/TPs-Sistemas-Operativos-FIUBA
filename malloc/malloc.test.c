...
successful_malloc_returns_non_null_pointer(void)
{
	...
	ASSERT_TRUE("[MALLOC - success]successful malloc returns non null pointer", var != NULL);
...
correct_copied_value(void)
{
	...
	ASSERT_TRUE("[MALLOC - copied value] allocated memory should contain the copied value",
...
correct_amount_of_mallocs(void)
{
	...
	ASSERT_TRUE("[MALLOC - stats] amount of mallocs should be one", stats.mallocs == 1);
}
...
correct_amount_of_frees(void)
{
	...
	ASSERT_TRUE("[MALLOC - stats] amount of frees should be one", stats.frees == 1);
}
...
static void fail_if_malloc_exceeds_memory_limit(void) {
	struct malloc_stats stats;
...
static void fail_if_requests_negative_amount(void) {
	struct malloc_stats stats;
...
static void alloc_min_size_if_requested_less(void) {
	struct malloc_stats stats;

	char *var = malloc(1);

	get_stats(&stats);
	ASSERT_TRUE("[MALLOC - min] amount of requested memory should be 64 (min size)", var == NULL);

	free(var);
}

static void fail_if_requested_zero(void) {
	struct malloc_stats stats;

	char *var = malloc(0);
	get_stats(&stats);

	ASSERT_TRUE("[MALLOC - requested 0] should return NULL pointer", var == NULL);
	free(var);
}
